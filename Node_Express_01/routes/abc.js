			...
			function (require, module, exports) {
				...
				function launchAnimations(
						...
						setTimeout(function () {
							if (index === nodes.length) {
								...
								if (object) {
									document.getElementById(
										board.start
									).className = "visitedStartNodePurple";
								} else {
									if (
										document.getElementById(board.start)
											.className !==
										"visitedStartNodePurple"
									) {
										document.getElementById(
											board.start
										).className = "visitedStartNodeBlue";
									}
								}
								if (
					...
					function change(currentNode, previousNode, bidirectional) {
						let currentHTMLNode = document.getElementById(
							currentNode.id
						);
						...
						if (previousNode) {
							let previousHTMLNode = document.getElementById(
								previousNode.id
							);
							if (
								!relevantClassNames.includes(
			...
			function (require, module, exports) {
							...
							currentElement.onmouseup = () => {
								if (this.buttonsOn) {
									...
									if (board.pressedNodeStatus === "target") {
										...
										board.pressedNodeStatus === "start"
									) {
										board.start = currentId;
									} else if (
										board.pressedNodeStatus === "object"
									) {
							...
							currentElement.onmouseenter = () => {
								if (this.buttonsOn) {
									if (
										...
										if (
											board.pressedNodeStatus === "target"
										) {
											board.target = currentId;
											if (board.algoDone) {
												board.redoAlgorithm();
						...
						if (this.previouslySwitchedNode) {
							...
							this.previouslySwitchedNode.weight =
								this.previouslySwitchedNodeWeight === 15
									...
									: 0;
							this.previouslySwitchedNode = null;
							this.previouslySwitchedNodeWeight =
								currentNode.weight;
				...
				Board.prototype.drawShortestPath = function (
					...
					if (this.currentAlgorithm !== "bidirectional") {
						currentNode =
							this.nodes[this.nodes[targetNodeId].previousNode];
						if (object) {
							while (currentNode.id !== startNodeId) {
								this.objectShortestPathNodesToAnimate.unshift(
									currentNode
								);
								currentNode =
									this.nodes[currentNode.previousNode];
							}
						} else {
							while (currentNode.id !== startNodeId) {
								this.shortestPathNodesToAnimate.unshift(
									currentNode
								);
								document.getElementById(
									currentNode.id
						...
						if (
							...
							while (secondCurrentNode.id !== targetNodeId) {
								...
								if (
									...
									if (
										secondCurrentNode.otherdirection ===
										"left"
									) {
									...
									this.nodes[this.target].direction =
										getDistance(
											secondCurrentNode,
											this.nodes[this.target]
										)[2];
				...
				Board.prototype.drawShortestPathTimeout = function (
					...
					if (board.currentAlgorithm !== "bidirectional") {
						currentNode =
							board.nodes[board.nodes[targetNodeId].previousNode];
						if (object) {
							board.objectShortestPathNodesToAnimate.push(
								"object"
							);
							...
							while (currentNode.id !== startNodeId) {
								...
								currentNode =
									board.nodes[currentNode.previousNode];
							}
						}
					} else {
						if (
							...
							currentNode =
								board.nodes[
									board.nodes[board.middleNode].previousNode
								];
							secondCurrentNode =
								board.nodes[
							...
							if (secondCurrentNode.id === board.target) {
								board.nodes[board.target].direction =
									getDistance(
										board.nodes[board.middleNode],
										board.nodes[board.target]
									)[2];
							...
							if (object) {
								...
								currentNodesToAnimate = [];
								board.nodes[board.middleNode].direction =
									getDistance(
										currentNode,
										board.nodes[board.middleNode]
									)[2];
				...
				Board.prototype.clearPath = function (clickedButton) {
					if (clickedButton) {
						...
						let object = this.numberOfObjects
							? this.nodes[this.object]
							: null;
						start.status = "start";
						...
						if (object) {
							...
							document.getElementById(object.id).className =
								"object";
						}
					}
				...
				Board.prototype.reset = function (objectNotTransparent) {
					...
					document.getElementById(this.start).className =
						"startTransparent";
				...
				Board.prototype.changeStartNodeImages = function () {
					...
					if (unweighted.includes(this.currentAlgorithm)) {
						...
						for (let i = 0; i < 14; i++) {
							let j = i.toString();
							let backgroundImage =
								document.styleSheets["1"].rules[j].style
									.backgroundImage;
					...
					if (this.currentAlgorithm === "bidirectional") {
						...
						).className = "navbar-inverse navbar-nav disabledA";
					} else {
						document.getElementById("bombLegend").className = "";
						document.getElementById(
							"startButtonAddObject"
					...
					if (!this.buttonsOn) {
						...
						document.getElementById(
							...
							if (this.numberOfObjects) {
								...
								document.getElementById(
									"startButtonAddObject"
								).innerHTML = '<a href="#">Add a Bomb</a></li>';
								document.getElementById(
									objectNodeId
								).className = "unvisited";
								this.object = null;
						...
						document.getElementById(
							"startButtonClearBoard"
						).onclick = () => {
							document.getElementById(
								"startButtonAddObject"
							).innerHTML = '<a href="#">Add Bomb</a></li>';
							...
							let textHeight =
								document.getElementById("mainText")
									.clientHeight +
								document.getElementById("algorithmDescriptor")
									.clientHeight;
						...
						document.getElementById(
							"startButtonClearWalls"
						).onclick = () => {
							this.clearWalls();
						};

						document.getElementById(
							"startButtonClearPath"
						).onclick = () => {
							this.clearPath("clickedButton");
						};
							...
							if (this.currentAlgorithm !== "bidirectional") {
								if (innerHTML.includes("Add")) {
									...
									if (
										...
										this.numberOfObjects = 1;
										this.nodes[objectNodeId].status =
											"object";
										document.getElementById(
											objectNodeId
										).className = "object";
						...
						document.getElementById(
							"actualStartButton"
						).style.backgroundColor = "rgb(185, 15, 15)";
					}
				};
			...
			function (require, module, exports) {
				...
				function bidirectional(
					...
					while (
						...
						let secondCurrentNode = closestNodeTwo(
							nodes,
							unvisitedNodesTwo
						);
						while (
							...
							if (secondCurrentNode.status === "wall")
								secondCurrentNode = closestNodeTwo(
									nodes,
									unvisitedNodesTwo
								);
						}
					...
					for (let i = 0; i < unvisitedNodes.length; i++) {
						if (
							...
							currentClosest.distance >
								nodes[unvisitedNodes[i]].distance
						) {
							currentClosest = nodes[unvisitedNodes[i]];
							index = i;
						}
